...
from batchgenerators.augmentations.utils import (
    create_zero_centered_coordinate_mesh, interpolate_img)
...
def batch_crop(volume, patch_size, seg=None, patch_ids=None):
    ...
    if patch_ids is None:
        ...
        if seg is not None:
            return volume[..., x_random:x_random + patch_size[0],
                   y_random:y_random + patch_size[1],
                   z_random:z_random + patch_size[2]], \
                   seg[..., x_random:x_random + patch_size[0],
                   y_random:y_random + patch_size[1],
                   z_random:z_random + patch_size[2]], \
                   [x_random, x_random + patch_size[0], y_random,
                    y_random + patch_size[1],
                    z_random, z_random + patch_size[2]]
        else:
            return volume[..., x_random:x_random + patch_size[0],
                   y_random:y_random + patch_size[1],
        ...
        for rep, vs, ps in zip(repeats, shape, patch_size):
            ...
            if final_index == 0:
                ...
                start_list = [start_point for start_point in
                              range(0, final_index, step)]
        ...
        _p = patch_ids
        if seg is not None:
            return volume[..., _s[0][_p[0]]:_s[0][_p[0]] + patch_size[0],
                   _s[1][_p[1]]:_s[1][_p[1]] + patch_size[1],
                   _s[2][_p[2]]:_s[2][_p[2]] + patch_size[2]], \
                   seg[..., _s[0][_p[0]]:_s[0][_p[0]] + patch_size[0],
                   _s[1][_p[1]]:_s[1][_p[1]] + patch_size[1],
                   _s[2][_p[2]]:_s[2][_p[2]] + patch_size[2]], \
                   [_s[0][_p[0]], _s[0][_p[0]] + patch_size[0], _s[1][_p[1]],
                    _s[1][_p[1]] + patch_size[1],
                    _s[2][_p[2]], _s[2][_p[2]] + patch_size[2]]
        else:
            return volume[..., _s[0][_p[0]]:_s[0][_p[0]] + patch_size[0],
                   _s[1][_p[1]]:_s[1][_p[1]] + patch_size[1],
    ...
    for sample_id in range(vol.shape[0]):
        for channel_id in range(vol.shape[1]):
            data_result[sample_id, channel_id] = interpolate_img(
                vol[sample_id, channel_id], coords,
                order=bspline_order,
...
def rotation(i_volume, shape, angle):
    ...
    coords = rotate_coords_3d(coords, angle_x=angle[0] * p,
                              angle_y=angle[1] * p, angle_z=angle[2] * p,
                              is_reverse=False)
    # print("forward rotate coords", coords)
    coords = shift_center(coords, shape)
    new_volume = batch_deform_3d(i_volume, coords, bspline_order=0,
                                 border_mode="nearest",
                                 constant_val=0)
    return new_volume[0, :]
...
def random_cutout(mask_array, num=1, size=(5, 5, 5), ):
    ...
    for i in range(num):
        ...
        mask_array[x:min(x + size[0], patch_size[0]),
        y:min(y + size[1], patch_size[1]),
        z:min(z + size[2], patch_size[2])] = 0
    return mask_array
...
if __name__ == "__main__":
    ...
    if is_translate:
        ...
        recovered_coords = translate_coords_3d(recovered_coords, t_x=-5.3,
                                               t_y=-6.2, t_z=-3.1)
        ...
        new_volume = batch_deform_3d(i_volume.detach().numpy(), coords,
                                     bspline_order=0,
                                     border_mode="nearest",
                                     constant_val=0)
        recovered_volume = batch_deform_3d(new_volume, recovered_coords,
                                           bspline_order=0,
                                           border_mode="nearest",
                                           constant_val=0)
    ...
    if is_rotate:
        ...
        coords = create_zero_centered_coordinate_mesh(shape)
        print("initial coords:", coords)
        coords = rotate_coords_3d(coords, angle_x=0.25 * p, angle_y=0.33 * p,
                                  angle_z=0.66 * p,
                                  is_reverse=False)
        print("forward rotate coords", coords)
        coords = shift_center(coords, shape)
        print("forward shifted coords", coords)
        recovered_coords = create_zero_centered_coordinate_mesh(shape)
        recovered_coords = rotate_coords_3d(recovered_coords,
                                            angle_x=-0.25 * p,
                                            angle_y=-0.33 * p,
                                            angle_z=-0.66 * p,
                                            is_reverse=True)
    ...
    if is_affine:
        ...
        out_volume = affine_transformation(i_volume,
                                           radius=(0.25 * p, 0.33 * p, 0.16 * p),
                                           translate=(2.1, 0, 5.7),
                                           scale=(2, 2, 2),
                                           bspline_order=0,
                                           border_mode="nearest",
                                           constant_val=0, is_reverse=False)
        save(i_volume.squeeze().detach().numpy(),
             filename="./affine_in.nii.gz")
        save(out_volume.squeeze().detach().numpy(),
             filename="./affine_out.nii.gz")
        rec_volume = affine_transformation(out_volume,
                                           radius=(-0.25 * p, -0.33 * p, -0.16 * p),
                                           translate=(-2.1, 0, -5.7),
                                           scale=(0.5, 0.5, 0.5),
                                           bspline_order=0,
                                           border_mode="nearest",
                                           constant_val=0, is_reverse=True)
        save(rec_volume.squeeze().detach().numpy(),
             filename="./affine_rec.nii.gz")
    ...
    if is_volume:
        ...
        i_volume = i_volume.reshape(1, 1, i_volume.shape[0], i_volume.shape[1],
                                    i_volume.shape[2])
        i_mask = i_mask.reshape(1, 1, i_mask.shape[0], i_mask.shape[1],
                                i_mask.shape[2])
        radius = np.array((0.05 * p, 0.08 * p, 0.05 * p))
        translate = np.array((0., 0., 0.))
        scale = np.array((0.9, 0.9, 0.9))
        out_volume = affine_transformation(i_volume, radius=radius,
                                           translate=translate, scale=scale,
                                           bspline_order=3,
                                           border_mode="constant",
                                           constant_val=0, is_reverse=False)
        out_mask = affine_transformation(i_mask, radius=radius,
                                         translate=translate, scale=scale,
                                         bspline_order=0,
                                         border_mode="nearest", constant_val=0,
                                         is_reverse=False)
        rec_volume = affine_transformation(out_volume, radius=-radius,
                                           translate=-translate, scale=scale,
                                           bspline_order=3,
                                           border_mode="constant",
                                           constant_val=0, is_reverse=True)
        rec_mask = affine_transformation(out_mask, radius=-radius,
                                         translate=-translate, scale=scale,
                                         bspline_order=0,
                                         border_mode="nearest", constant_val=0,
                                         is_reverse=True)
        ...
        save(i_volume.squeeze(), filename="./affine_result/in_vol.nii.gz",
             hdr=h1)
        save(i_mask.squeeze(), filename="./affine_result/in_msk.nii.gz",
             hdr=h2)

    ...
    def __init__(self, num_input_features, num_output_channels, drop_rate):
        ...
        self.add_module('conv1',
                        nn.Conv3d(num_input_features,
                                  num_output_channels, kernel_size=3, stride=1,
                                  padding=1,
                                  bias=False)),
...
class SinglePathDenseNet(BaseModel):
    def __init__(self, in_channels, classes=4, drop_rate=0.1,
                 return_logits=True, early_fusion=False):
        ...
        if early_fusion:
            block = _HyperDenseBlockEarlyFusion(
                num_input_features=in_channels, drop_rate=drop_rate)
            if in_channels == 52:
            ...
            block = _HyperDenseBlock(num_input_features=in_channels,
                                     drop_rate=drop_rate)
            if in_channels == 2:
        ...
        self.denseblock1 = block
        self.features.add_module('conv1x1_1',
                                 nn.Conv3d(total_conv_channels,
                                           256, kernel_size=1, stride=1,
                                           padding=0,
                                           bias=False))
        self.features.add_module('bn_1', nn.BatchNorm3d(256))
        self.features.add_module('relu_1', nn.ReLU(inplace=True))
        # self.features.add_module('drop_1', nn.Dropout(p=0.5))

        self.features.add_module('conv1x1_2',
                                 nn.Conv3d(256,
                                           128, kernel_size=1, stride=1,
                                           padding=0,
                                           bias=False))
        self.features.add_module('bn_2', nn.BatchNorm3d(128))
        self.features.add_module('relu_2', nn.ReLU(inplace=True))
        # self.features.add_module('drop_2', nn.Dropout(p=0.5))

        self.features.add_module('conv1x1_3',
                                 nn.Conv3d(128,
                                           64, kernel_size=1, stride=1,
                                           padding=0,
                                           bias=False))

        self.features.add_module('bn_3', nn.BatchNorm3d(64))
    ...
    def forward(self, x):
        dense_out = self.denseblock1(x)
        features = self.features(dense_out)
        if self.return_logits:
            out = self.classifier(features)
            return out, dense_out[0]

        else:
            return features, dense_out[0]

    def test(self, device='cpu'):

        input_tensor = torch.rand(1, self.input_channels, 64, 64, 64).to(
            device)
        ideal_out = torch.rand(1, self.num_classes, 64, 64, 64)
        out = self.forward(input_tensor)
        # assert ideal_out.shape == out.shape
        summary(self.to(torch.device(device)),
                (self.input_channels, 64, 64, 64), device=device)
...
class DualPathDenseNet(BaseModel):
    def __init__(self, in_channels, classes=4, drop_rate=0, fusion='concat'):
        ...
        if self.input_channels == 2:
            # here!!!!
            self.stream_1 = SinglePathDenseNet(in_channels=1,
                                               drop_rate=drop_rate,
                                               classes=classes,
                                               return_logits=False,
                                               early_fusion=True)
            self.stream_2 = SinglePathDenseNet(in_channels=1,
                                               drop_rate=drop_rate,
                                               classes=classes,
                                               return_logits=False,
                                               early_fusion=True)
        ...
        if self.input_channels == 3:
            ...
            self.stream_2 = SinglePathDenseNet(in_channels=1,
                                               ...
                                               classes=classes,
                                               return_logits=False)
            self.stream_3 = SinglePathDenseNet(in_channels=1,
                                               drop_rate=drop_rate,
                                               classes=classes,
                                               return_logits=False)
        ...
        self.classifier = nn.Sequential()
        self.classifier.add_module('classifier',
                                   nn.Conv3d(in_classifier_channels,
                                             classes, kernel_size=1, stride=1,
                                             padding=0,
                                             bias=False))
    ...
    def forward(self, multi_channel_medical_img):
        ...
        if channels != self.input_channels:
            print(
                "Network channels does not match input channels, check your model/input!")
            return None
        else:
            if self.input_channels == 2:
                in_stream_1 = multi_channel_medical_img[:, 0, ...].unsqueeze(
                    dim=1)
                in_stream_2 = multi_channel_medical_img[:, 1, ...].unsqueeze(
                    dim=1)
                ...
                if self.fusion == 'concat':
                    concat_features = torch.cat(
                        (output_features_t1, output_features_t2), dim=1)
                ...
                in_stream_2 = multi_channel_medical_img[:, 1, ...].unsqueeze(
                    dim=1)
                in_stream_3 = multi_channel_medical_img[:, 2, ...].unsqueeze(
                    dim=1)
                output_features_t1 = self.stream_1(in_stream_1)
                output_features_t2 = self.stream_2(in_stream_2)
                ...
                if self.fusion == 'concat':
                    concat_features = torch.cat(
                        (output_features_t1, output_features_t2,
                         output_features_t3), dim=1)
                    return self.classifier(concat_features)
                else:
    ...
    def test(self, device='cpu'):
        input_tensor = torch.rand(1, self.input_channels, 96, 160, 160)
        ideal_out = torch.rand(1, self.num_classes, 96, 160, 160)
        out = self.forward(input_tensor)
        assert ideal_out.shape == out.shape
        summary(self.to(torch.device(device)),
                (self.input_channels, 96, 160, 160), device=device)
        import torchsummaryX
        torchsummaryX.summary(self, input_tensor.to(device))
...
class DualSingleDenseNet(BaseModel):
    ...
    def __init__(self, in_channels, classes=4, drop_rate=0.5, ):
        ...
        if self.input_channels == 2:
            self.early_conv_1 = _HyperDenseLayer(num_input_features=1,
                                                 num_output_channels=25,
                                                 drop_rate=drop_rate)
            self.early_conv_2 = _HyperDenseLayer(num_input_features=1,
                                                 num_output_channels=25,
                                                 drop_rate=drop_rate)
            single_path_channels = 52
            self.stream_1 = SinglePathDenseNet(
                in_channels=single_path_channels, drop_rate=drop_rate,
        ...
        if self.input_channels == 3:
            ...
            self.early_conv_2 = _HyperDenseLayer(num_input_features=1,
                                                 num_output_channels=25,
                                                 drop_rate=0)
            self.early_conv_3 = _HyperDenseLayer(num_input_features=1,
                                                 num_output_channels=25,
                                                 drop_rate=0)
...
if __name__=="__main__":
    model = SinglePathDenseNet(1).cuda()
    model = model
    model.test(device="cuda")

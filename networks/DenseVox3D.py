...
try:
    ...
    from DenseNet3D import BaseModel
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size,
        ...
        self.add_module('conv1',
                        nn.Conv3d(num_input_features, bn_size *
...
class DenseVoxelNet(BaseModel):
    ...
    def __init__(self, in_channels=1, classes=3):
        ...
        self.conv_init = nn.Conv3d(in_channels, num_input_features,
                                   kernel_size=1, stride=2, padding=0,
                                   bias=False)
        self.dense_1 = _DenseBlock(num_layers=12,
                                   num_input_features=num_input_features,
                                   bn_size=1, growth_rate=12)
        self.trans = _Transition(self.dense_1_out_features,
                                 self.dense_1_out_features)
        self.dense_2 = _DenseBlock(num_layers=12,
                                   num_input_features=self.dense_1_out_features,
                                   bn_size=1,
                                   growth_rate=12)
        self.up_block = _Upsampling(self.dense_2_out_features,
                                    self.up_out_features)
        self.conv_final = nn.Conv3d(self.up_out_features, classes,
                                    kernel_size=1, padding=0, bias=False)
        self.transpose = nn.ConvTranspose3d(self.dense_1_out_features,
                                            self.up_out_features,
                                            kernel_size=2, padding=0,
                                            output_padding=0,
                                            stride=2)
    ...
    def test(self, device='cpu'):
        ...
        summary(self.to(torch.device(device)),
                (self.in_channels, 96, 160, 160), device=device)
        b, c = self.forward(a.to(device))

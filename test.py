...
task_name_id_dict = {
    "full": 0, "spleen": 1, "kidney": 2, "liver": 4, "pancreas": 5, "heart": 0
}
                ...
                with torch.no_grad():
                    if condition > 0:
                        condition = torch.tensor(
                            [condition], dtype=torch.long, device="cuda"
                        )
...
def cal_metric(gt, pred, cal_hd95=False, cal_asd=False, spacing=None):
    if pred.sum() > 0 and gt.sum() > 0:
        ...
        # Add Jaccard metric calculation
        ...
        if cal_asd:
            ...
            asd = 0.0
        # Organize the array in the order of Dice, ASD, HD95, Jaccard
        return np.array([dice, asd, hd95, jc])
    else:
        # Update the default value accordingly
        return np.array([0.0, 150, 150, 0.0])


def test_all_case_BCV_old(
    net, test_list="full_test.list", num_classes=4,
    patch_size=(48, 160, 160), stride_x=32,
    stride_y=32, stride_z=24,
    condition=-1, method="regular", cal_hd95=True,
    cal_asd=True, cut_lower=-68, cut_upper=200,
    ...
    for i, image_path in enumerate(tqdm(image_list)):
        ...
        for i in range(1, num_classes):
            metrics = cal_metric(
                label == i, prediction == i, cal_hd95=True,
                cal_asd=True, spacing=spacing
            )
            print(
                f"class:{class_name_list[i-1]}, Dice:{metrics[0]:.4f}, "
                f"ASD:{metrics[1]:.4f}, HD95:{metrics[2]:.4f}, "
                f"Jaccard:{metrics[3]:.4f}"
            )
            res_metric[class_name_list[i - 1]] = {
                'Dice': metrics[0], 'ASD': metrics[1], 'HD95': metrics[2],
                'Jaccard': metrics[3]
            }
        ...
        res_metric['Mean'] = {
            'Dice': each_metric[:, 0].mean(), 'ASD': each_metric[:, 1].mean(),
            'HD95': each_metric[:, 2].mean(), 'Jaccard': each_metric[:, 3].mean()
        }
...
if __name__ == '__main__':
    ...
    pred_save_path = "{}/Prediction_{}_iter{}_dice{{}}/".format(
        root_path, model_type, iteration, dice
    )
    if os.path.exists(pred_save_path):
        shutil.rmtree(pred_save_path)
    os.makedirs(pred_save_path)
    model = net_factory_3d(
        net_type=config['backbone'] if 'model2' not in model_path else config['backbone2'],
        in_chns=1,
    ...
    # Display the four indicators in the new order
    print(" | Mean Dice |  Mean ASD  | Mean HD95  | Mean Jaccard |")
    print(
        f" | {avg_metric[:, 0].mean():.4f}      "
